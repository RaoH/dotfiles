...
def "nu completion tag" [] {
    op item list --format json | from json | get tags --optional | uniq | sort --ignore-case --natural
}
...
def "nu completion account" [] {
    op account list --format json | from json | select --optional account_uuid email | rename value description | sort-by description --ignore-case --natural
}

def "nu completion document_item" [] {
    op item list --format json | from json | where category == "DOCUMENT" | select --optional id title | rename value description | sort-by description --ignore-case --natural
}

def "nu completion item" [] {
    op item list --format json | from json | select --optional id title additional_information | rename value description
